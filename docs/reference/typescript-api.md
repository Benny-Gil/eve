...
Tool-wide authoring helpers such as `defineTool`, `defineWorkflowTool`, `defineDynamic`, and `disableTool` come from `eve/tools`. Capability-specific definitions and helpers use their own subpaths (see [Built-in tools](../concepts/built-in-tools)): reusable definitions such as `bash` and `glob` come from `eve/tools/<name>`, `webSearch` comes from `eve/tools/web_search`, `experimental_workflow` comes from `eve/tools/workflow`, `sleep` comes from `eve/tools/sleep`, and approval policies and types come from `eve/tools/approval`. The route verbs `GET`/`HEAD`/`POST`/`PUT`/`PATCH`/`DELETE`/`OPTIONS`/`WS` plus `disableRoute` come from `eve/channels`, the channel auth helpers `localDev`/`vercelOidc`/`placeholderAuth` come from `eve/channels/auth`, and the inbound webhook primitives `verifyHmacSha256`/`verifyWebhookTimestamp`/`constantTimeCompare` plus the shared `WebhookVerifier` type come from `eve/channels/webhook`.
...
| `eve/channels/webhook`                                                      | `WebhookVerifier`, `verifyHmacSha256`, `verifyWebhookTimestamp`, `constantTimeCompare` |
